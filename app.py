...
Copyright (C) 2025 thadawilliams
...
def load_config():
    if os.path.exists(CONFIG_FILE):
        with open(CONFIG_FILE,'r') as f:
            return json.load(f)
    return {'dbPath':'/plex-db/com.plexapp.plugins.library.db','outputDir':REPORTS_DIR,'excludeDays':30,'selectedLibraries':{},'libraryTypes':{}}
...
def scan_libraries():
    config=load_config()
    db_path=config.get('dbPath','/plex-db/com.plexapp.plugins.library.db')
    if not os.path.exists(db_path):
        return jsonify({'error':'Database file not found'}),404
    try:
        conn=sqlite3.connect(db_path)
        cursor=conn.execute("SELECT id,name FROM library_sections ORDER BY name")
        libraries=[{'id':row[0],'name':row[1]} for row in cursor]
        conn.close()
...
def generate_reports():
    ...
    config=load_config()
    db_path=config.get('dbPath','/plex-db/com.plexapp.plugins.library.db')
    output_dir=config.get('outputDir',REPORTS_DIR)
    ...
    try:
        conn=sqlite3.connect(db_path)
        for lib_id,selected in selected_libraries.items():
            if not selected:
                continue
            lib_id=int(lib_id)
            lib_type=library_types.get(str(lib_id),'movie')
            cursor=conn.execute("SELECT name FROM library_sections WHERE id=?",(lib_id,))
            row=cursor.fetchone()
            if not row:
                continue
            lib_name=row[0]
            timestamp=datetime.now().strftime("%Y-%m-%d")
            sanitized_name="".join(c for c in lib_name if c.isalnum() or c in(' ','-','_')).rstrip()
            filename=f"{sanitized_name}_{timestamp}.csv"
            output_path=os.path.join(output_dir,filename)
            if lib_type=='movie':
                item_count=generate_movie_report(conn,lib_id,output_path,exclude_days)
            else:
                item_count=generate_tv_report(conn,lib_id,output_path,exclude_days)
            results.append({'library':lib_name,'filename':filename,'itemCount':item_count,'path':f'/api/download/{filename}'})
        conn.close()
        return jsonify({'reports':results})
    except Exception as e:
        return jsonify({'error':str(e)}),500

def generate_movie_report(conn,library_id,output_path,exclude_days):
    cutoff_timestamp=get_cutoff_timestamp(exclude_days)
    query="""SELECT mi.title,CASE WHEN mi.originally_available_at IS NOT NULL THEN strftime('%Y',datetime(mi.originally_available_at,'unixepoch')) ELSE NULL END as year,datetime(mi.added_at,'unixepoch','localtime') as date_added,COALESCE(COUNT(DISTINCT miv.id),0) as play_count,mp.file,mp.size FROM metadata_items mi LEFT JOIN metadata_item_views miv ON mi.guid=miv.guid LEFT JOIN media_items mdi ON mi.id=mdi.metadata_item_id LEFT JOIN media_parts mp ON mdi.id=mp.media_item_id WHERE mi.library_section_id=? AND mi.metadata_type=1 AND mi.added_at<? GROUP BY mi.id ORDER BY mi.added_at DESC"""
    cursor=conn.execute(query,(library_id,cutoff_timestamp))
    with open(output_path,'w',newline='',encoding='utf-8') as csvfile:
        writer=csv.writer(csvfile)
        writer.writerow(['Title','Year','Date Added','Play Count','File Path','File Size'])
        row_count=0
        for row in cursor:
            title,year,date_added,play_count,file_path,file_size=row
            writer.writerow([title,year if year else 'Unknown',date_added,play_count,file_path or 'Unknown',format_file_size(file_size)])
            row_count+=1
    return row_count

def generate_tv_report(conn,library_id,output_path,exclude_days):
    cutoff_timestamp=get_cutoff_timestamp(exclude_days)
    query="""SELECT show.title as show_title,season."index" as season_number,datetime(season.added_at,'unixepoch','localtime') as date_added,COUNT(DISTINCT episode.id) as total_episodes,COUNT(DISTINCT CASE WHEN miv.id IS NOT NULL THEN episode.id END) as watched_episodes FROM metadata_items show JOIN metadata_items season ON season.parent_id=show.id JOIN metadata_items episode ON episode.parent_id=season.id LEFT JOIN metadata_item_views miv ON episode.guid=miv.guid WHERE show.library_section_id=? AND show.metadata_type=2 AND season.metadata_type=3 AND episode.metadata_type=4 AND season.added_at<? GROUP BY show.id,season.id ORDER BY show.title,season."index" """
    cursor=conn.execute(query,(library_id,cutoff_timestamp))
    with open(output_path,'w',newline='',encoding='utf-8') as csvfile:
        writer=csv.writer(csvfile)
        ...
        row_count=0
        for row in cursor:
            show_title,season_num,date_added,total_eps,watched_eps=row
            watched_status='Yes' if watched_eps>0 else 'No'
            writer.writerow([show_title,f"Season {season_num}",watched_status,total_eps,watched_eps,date_added])
            row_count+=1
    return row_count
